...
export function ConversationList({ conversations, selected, onSelect, onToggleRail, onNewChat }: ConversationListProps) {
  ...
  return (
    <aside className="conversation-sidebar">
      <header className="sidebar-header"><div className="sidebar-title"><button className="icon-button sidebar-rail-toggle" onClick={onToggleRail} aria-label="Open navigation"><Icon name="menu" /></button><h1>Chats</h1></div><div><button className="icon-button" onClick={onNewChat} aria-label="New chat"><Icon name="compose" /></button><button className="icon-button" aria-label="More chat options"><Icon name="more" /></button></div></header>
      <div className="search-zone"><label className="search-field"><Icon name="search" size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search" aria-label="Search conversations" />{query && <button onClick={() => setQuery("")} aria-label="Clear search">×</button>}</label><button className="icon-button" aria-label="Filter conversations"><Icon name="filter" /></button></div>
