  ...
  return (
    <>
      <aside className="settings-sidebar">
        ...
        <div className="settings-nav" aria-label="Settings sections"><button className={page === "general" ? "selected" : ""} onClick={() => { setPage("general"); setDetailOpen(true); }}><Icon name="settings" /><span>General</span><Icon name="chevron" className="row-chevron" /></button><button className={page === "privacy" ? "selected" : ""} onClick={() => { setPage("privacy"); setDetailOpen(true); }}><Icon name="person" /><span>Privacy</span><Icon name="chevron" className="row-chevron" /></button><button className={page === "notifications" ? "selected" : ""} onClick={() => { setPage("notifications"); setDetailOpen(true); }}><Icon name="chat" /><span>Notifications</span><Icon name="chevron" className="row-chevron" /></button><button className={page === "appearance" ? "selected" : ""} onClick={() => { setPage("appearance"); setDetailOpen(true); }}><Icon name="appearance" /><span>Appearance</span><Icon name="chevron" className="row-chevron" /></button><button className={page === "devices" ? "selected" : ""} onClick={() => { setPage("devices"); setDetailOpen(true); }}><Icon name="phone" /><span>Linked devices</span><Icon name="chevron" className="row-chevron" /></button></div>
      </aside>
      <section className={`settings-content ${detailOpen ? "detail-open" : ""}`}>
        <header className="settings-content-header"><button className="icon-button settings-content-back" onClick={() => setDetailOpen(false)} aria-label="Back to settings"><Icon name="back" /></button><h2>{page === "profile" ? "Profile" : page === "general" ? "General" : page === "privacy" ? "Privacy" : page === "notifications" ? "Notifications" : page === "devices" ? "Linked devices" : "Appearance"}</h2></header>
        {page === "profile" ? <ProfilePane initials={initials} name={name} /> : page === "general" ? <GeneralPane phone={phone} onLogout={onLogout} /> : page === "privacy" ? <PlaceholderPane title="Privacy settings" description="Privacy controls are ready for your preferences in a future update." /> : page === "notifications" ? <PlaceholderPane title="Notifications" description="Notification controls are coming soon. Message toasts remain available in this demo." /> : page === "devices" ? <PlaceholderPane title="No linked devices" description="Linking additional Signal devices is available as a placeholder in this assignment build." /> : <AppearancePane theme={theme} setTheme={setTheme} />}
      </section>
    </>
  );
}
...
function PlaceholderPane({ title, description }: { title: string; description: string }) {
  return <div className="settings-simple-pane"><section className="settings-placeholder"><h3>{title}</h3><p>{description}</p><span>Coming soon</span></section></div>;
}
